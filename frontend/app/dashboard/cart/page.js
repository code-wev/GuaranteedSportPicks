...
export default function CartPage() {
  ...
  const { data: pickData, isLoading: isLoadingPick } =
    useGetMyPickPurchasesQuery();
  ...
  const rows = useMemo(() => {
    ...
    const picks = (pickData?.data || []).map((purchase) => {
      const pick =
        purchase.pickId && typeof purchase.pickId === "object"
          ? purchase.pickId
          : null;
      const awayTeam =
        pick?.away_team || purchase?.pickSnapshot?.awayTeam || "Unknown";
      const homeTeam =
        pick?.home_team || purchase?.pickSnapshot?.homeTeam || "Pick";
      ...
      <div className='flex items-start justify-between gap-4 flex-wrap'>
        <div>
          ...
          <p className='text-sm text-gray-500 mt-1'>
            See what you bought and each payment status (paid, authorized,
            pending, cancelled).
          </p>
        </div>
        <Link
