...
import { MdBookmarkBorder, MdOutlineShare } from "react-icons/md";
import "../blog.css";
...
export default function BlogDetails({ params }) {
  ...
  if (isLoading)
    return (
      <div className='text-center p-20 min-h-screen'>Loading article...</div>
    );
  if (isError || !data?.status)
    return (
      <div className='text-center p-20 min-h-screen'>Article not found.</div>
    );
    ...
    <div className='bg-white'>
      ...
      <div className='bg-[#F9FAFB] border-b border-gray-100'>
        <p className='text-sm text-gray-500 max-w-7xl mx-auto py-4 px-4'>
          Home / Blog /{" "}
          <span className='text-[#B91C1C] font-medium'>{article.category}</span>
        </p>
      </div>
      ...
      <div className='max-w-4xl mx-auto px-4 py-10 md:py-16'>
        ...
        <div className='flex items-center justify-between border-y border-gray-100 py-6 mb-8 flex-wrap gap-4'>
          ...
          <div className='flex items-center gap-4 text-gray-500'>
            <button
              onClick={handleShare}
              className='flex items-center gap-2 hover:text-[#B91C1C] transition-colors font-medium text-sm'>
        ...
        <div className='mt-16 p-8 rounded-2xl border border-gray-100 bg-gray-50 flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left'>
          ...
          <div className='flex-grow'>
            ...
            <p className='text-gray-600 leading-relaxed mb-4'>
              Our expert analysts provide the most in-depth sports betting
              insights, leveraging years of experience and advanced data metrics
              to give you the winning edge in every game.
            </p>
