        ...
        {isLoading ? (
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-7'>
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className='rounded-[18px] bg-white border border-[#00000012] h-[380px] animate-pulse'
              />
            ))}
          </div>
        ) : articles.length === 0 ? (
  ...
  return (
    <article className='group rounded-[18px] bg-white border border-[#00000012] shadow-[0_14px_36px_rgba(0,0,0,0.08)] overflow-hidden'>
      <div className='relative w-full h-45 bg-gray-100'>
        ...
        <div
          className={`pointer-events-none absolute inset-0 bg-linear-to-b ${gradient} opacity-0 group-hover:opacity-[0.06] transition`}
        />
      </div>
