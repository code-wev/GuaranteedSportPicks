...
export default function SelectPackage({ selectedPackage, onSelectPackage, seasonalDays, setSeasonalDays, seasonalPrice, setSeasonalPrice, validationErrors }) {
  const packages = [
    {
      id: 1,
      name: "DAILY",
    ...
    },
    {
      ...
      price: null,
      features: ["Custom Days", "Custom Price", "Flexible Access", "Personalized Picks"],
      popular: false
    }
  ];
    ...
    <div className="bg-white rounded-xl p-5 shadow-sm mb-8">
      ...
      {selectedPackage === "SEASONAL" && (
        <div className="mt-4 space-y-2">
          {(validationErrors?.seasonalDays || validationErrors?.seasonalPrice) && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
              {validationErrors.seasonalDays && (
                <p className="text-red-600 text-sm">{validationErrors.seasonalDays}</p>
              )}
              {validationErrors.seasonalPrice && (
                <p className="text-red-600 text-sm">{validationErrors.seasonalPrice}</p>
              )}
            </div>
          )}
        </div>
